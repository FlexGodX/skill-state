...
export class SkillStateLlmAdapter extends LlmAdapter {
  ...
  async *stream(options) {
    ...
    const requestControl = createRequestSignal(options.signal, this.config.timeoutMs)
    try {
      ...
      if (requestControl.timedOut()) {
        throw new LlmError(`skill-state gateway request timed out after ${this.config.timeoutMs}ms`, 'TIMEOUT', { cause: error })
      }
      throw error
    } finally {
