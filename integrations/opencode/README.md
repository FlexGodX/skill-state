...
session events, chat messages, and tool execution before/after events. It writes metadata-only
observations to `.skill-state/observations.ndjson` relative to the OpenCode process unless
`SKILL_STATE_OBSERVATION_SINK` names a file or HTTP endpoint. Tool arguments and results are not copied.
